...
void Projection::apply(const snow::TrajectoryEvent &event) {
  ...
  if (event.type == "user/message") {
    ...
    items_[*streaming_assistant_].content += event.data.value("content", "");
    ...
    items_.push_back(
        {event.data.value("tool_call_id", tokmon::make_uuid()), ItemKind::tool,
    ...
    const auto iterator = std::ranges::find(items_, id, &ConversationItem::id);
    if (iterator != items_.end()) {
      ...
      iterator->metadata = event.data.value("metadata", tokmon::Json::object());
      const auto name = event.data.value("name", "unknown");
      if (name == "shell")
        iterator->title = "Terminal / shell";
      if (name == "write_file")
        iterator->title = "Diff / write_file";
    } else {
      items_.push_back(
          {id, ItemKind::tool, "Tool / " + event.data.value("name", "unknown"),
           event.data.value("content", ""), event.data.value("status", "error"),
           event.seq, event.data.value("metadata", tokmon::Json::object())});
    }
  } else if (event.type == "approval/request") {
    items_.push_back(
        {event.data.value("tool_call_id", tokmon::make_uuid()),
         ItemKind::approval, "Approval / " + event.data.value("name", "tool"),
         event.data
             .value("canonical_arguments",
                    event.data.value("arguments", tokmon::Json::object()))
             .dump(2),
         "waiting", event.seq});
    ...
    if (iterator != items_.end()) {
      iterator->status =
          event.data.value("approved", false) ? "approved" : "denied";
      iterator->source_seq = event.seq;
    }
    ...
    items_.push_back({tokmon::make_uuid(), ItemKind::artifact,
                      ...
                      event.seq, artifacts});
  } else if (event.type == "agent/error" || event.type == "agent/cancelled") {
...
std::vector<snow::TrajectoryEvent> Projection::event_snapshot() const {
  std::lock_guard lock(mutex_);
  return events_;
}
